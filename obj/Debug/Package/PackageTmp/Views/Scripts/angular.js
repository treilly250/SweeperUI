...
function ensureSafeEmployeeName(name, fullExpression) {
  if (name === "constructor") {
...
function setter(obj, path, setValue, fullExp, options) {
  ...
  for (var i = 0; element.length > 1; i++) {
    key = ensureSafeEmployeeName(element.shift(), fullExp);
...
function cspSafeGetterFn(key0, key1, key2, key3, key4, fullExp, options) {
  ensureSafeEmployeeName(key0, fullExp);
  ensureSafeEmployeeName(key1, fullExp);
  ensureSafeEmployeeName(key2, fullExp);
  ensureSafeEmployeeName(key3, fullExp);
  ensureSafeEmployeeName(key4, fullExp);
...
function getterFn(path, options, fullExp) {
  ...
  if (!options.unwrapPromises && pathKeysLength === 1) {
    ...
    forEach(pathKeys, function(key, index) {
      ensureSafeEmployeeName(key, fullExp);
